...
### Core optimization technology: Google OR-Tools

Use **Google OR-Tools** as the main optimization library for discrete and constrained business decisions.

OR-Tools should be treated as a **first-class dependency**, not an optional afterthought.

Primary use cases:

```text
1. Inventory ordering under a cash budget
2. Supplier selection under lead-time/reliability/cost tradeoffs
3. Menu selection under ingredient availability constraints
4. Staffing level selection under demand, service-risk, and cash constraints
5. Promotion/marketing decisions under capacity and inventory constraints
```
...
Add it to `requirements-hybrid.txt`:

```text
ortools>=9.9
```

Use OR-Tools mainly through:
...
```text
Use CP-SAT for binary/integer decisions:
- choose this order or not
- choose this supplier or not
- choose this dish or not
- choose this staff level or not
```

Use greedy fallback if OR-Tools is unavailable or exceeds the time budget.
...
If `ORTOOLS_AVAILABLE` is false:

```text
fall back to deterministic greedy heuristics
...
```text
Inventory optimization: max 0.2 to 0.8 seconds
Menu optimization: max 0.2 to 0.5 seconds
Staffing optimization: max 0.1 to 0.3 seconds
Overall strategy tick budget: never let optimization consume the 30-second game tick
```

Example CP-SAT time limit:
...
Reason for using one worker:

```text
Keeps runtime predictable inside the simulator tick budget.
```
...
## OR-Tools optimization design

Create `optimization.py`.

This module should expose small, safe optimization functions used by the other modules.

Recommended file:

```text
agents/hybrid_operator/optimization.py
```

Public functions:

```python
def solve_inventory_orders_ortools(
    candidate_orders: list[dict],
    budget_cents: int,
    max_orders: int,
    must_have_ingredients: set[str],
    solver_time_limit_seconds: float = 0.5,
) -> list[dict]:
    ...


def solve_supplier_allocation_ortools(
...
def solve_menu_selection_ortools(
    candidate_dishes: list[dict],
    min_dishes: int,
    max_dishes: int,
    solver_time_limit_seconds: float = 0.3,
) -> list[str]:
    ...


def solve_staff_level_ortools(
    candidate_levels: list[dict],
    solver_time_limit_seconds: float = 0.2,
) -> int:
    ...
```
...
```text
- return a valid deterministic fallback if OR-Tools is unavailable
- use integer cents instead of floats for money
- set a strict solver time limit
- avoid crashing the main strategy loop
- return only candidates that the validator can accept
```

---

### OR-Tools problem 1: inventory ordering

This is the most important optimization problem.

Input candidate order format:
...
```python
x_i = 1 if candidate order i is selected, else 0
```
...
Example integer objective:

```python
value_i = (
    1000 * criticality
    + 300 * reliability_score_scaled
    + 100 * delivery_speed_score
    - 200 * waste_risk
)
```
...
sum(x_i) <= max_orders
for must-have ingredients:
    at least one selected order should cover the ingredient if feasible
avoid duplicate supplier+ingredient candidates unless explicitly allowed
```
...
def solve_inventory_orders_ortools(candidate_orders, budget_cents, max_orders, must_have_ingredients, solver_time_limit_seconds=0.5):
    if not ORTOOLS_AVAILABLE or not candidate_orders or budget_cents <= 0:
        return greedy_inventory_orders(candidate_orders, budget_cents, max_orders, must_have_ingredients)

    model = cp_model.CpModel()
    x = [model.NewBoolVar(f"order_{i}") for i in range(len(candidate_orders))]

    model.Add(sum(x[i] * int(candidate_orders[i]["cost_cents"]) for i in range(len(candidate_orders))) <= int(budget_cents))
    model.Add(sum(x) <= int(max_orders))

    for ingredient in must_have_ingredients:
        idxs = [i for i, o in enumerate(candidate_orders) if o["ingredient"] == ingredient]
        if idxs:
            model.Add(sum(x[i] for i in idxs) >= 1)

    values = []
    for o in candidate_orders:
        criticality = int(round(o.get("criticality", 0) * 100))
        reliability = int(round(o.get("reliability_score", 0.75) * 100))
        delivery_speed = max(0, 10 - int(o.get("delivery_days", 7))) * 10
        waste_penalty = int(round(o.get("waste_risk", 0) * 100))
        values.append(criticality + 3 * reliability + delivery_speed - 2 * waste_penalty)

    model.Maximize(sum(x[i] * values[i] for i in range(len(candidate_orders))))
    ...
    return [candidate_orders[i] for i in range(len(candidate_orders)) if solver.Value(x[i]) == 1]
```

Greedy fallback:

```text
1. Sort must-have candidates by value/cost.
2. Select one feasible order for each must-have ingredient.
3. Fill remaining budget by criticality/cost.
4. Stop before violating budget or max_orders.
```

---

### OR-Tools problem 2: supplier allocation

Use OR-Tools when several suppliers can provide the same ingredient.

Decision:

```text
choose supplier and quantity bucket for each ingredient
```

Simpler hackathon version:

```text
Generate candidate supplier orders first.
Then let inventory-order optimization choose the best combination.
```

This avoids a complex continuous quantity problem.
...
For each ingredient:
  for each valid supplier:
    for quantity in [min_order, target_qty, target_qty * 1.25]:
      create candidate order
```

Then send all candidates into `solve_inventory_orders_ortools`.

This is usually enough.

---

### OR-Tools problem 3: staffing level

For staffing, OR-Tools is optional but clean.
...
```text
choose exactly one staff level
```

Objective:

```text
maximize service_score - cash_penalty - overstaff_penalty
```
...
```text
3 <= level <= 15
projected cash after staff and core orders >= reserve when possible
```

For this hackathon, a deterministic staffing formula is acceptable. OR-Tools staffing is a nice improvement but not mandatory.

---

### OR-Tools problem 4: menu selection

Use OR-Tools if supply shocks make menu selection non-trivial.

Decision:

```text
x_dish = 1 if dish is active
```
...
```text
sum(x_dish) >= 5
sum(x_dish) <= 8 or 10, depending on strategy
must include at least one cheap/popular staple if available
avoid selecting too many dishes that depend on the same scarce ingredient
```

This helps during supply shocks and hidden scenarios.

---

### OR-Tools problem 5: promotions and marketing

Promotions are low-dimensional, so do not over-engineer.
...
Only use OR-Tools for promotions if you are already optimizing:

```text
staff + inventory + marketing under a shared cash budget
```

Otherwise keep promotion logic rule-based.

---

### OR-Tools engineering rules

Codex must follow these rules:

```text
- OR-Tools is used only inside small helper functions.
- The main agent must never depend on solver success.
- Every solver call has a strict time limit.
- Every solver function has a greedy fallback.
- Money is represented in integer cents.
- Quantities are bucketed to avoid continuous optimization complexity.
- The final action validator still checks all solver outputs.
```

Add tests:

```text
test_inventory_ortools_respects_budget
test_inventory_ortools_selects_must_have_ingredient
test_inventory_ortools_falls_back_without_ortools
test_menu_ortools_returns_at_least_5_dishes
test_staff_ortools_returns_one_valid_level
```
...
agent.py, state.py, memory.py, metrics.py, risk.py, scenario.py, planner.py, pricing.py, staffing.py, inventory.py, optimization.py, suppliers.py, satisfaction.py, menu.py, validator.py, constants.py, logging_utils.py, prompts.py.
...
[ ] OR-Tools inventory optimizer respects budget and has greedy fallback.
[ ] OR-Tools solver calls have strict time limits.
